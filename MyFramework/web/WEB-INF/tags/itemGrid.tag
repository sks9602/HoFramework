...
%> 		
	{ 	layout: { type : 'table' , columns: <%= HoValidator.isNotEmpty(title) ? 2 : 1 %> , tdAttrs: {valign: 'top'} }, 
		border : 0,
		defaults: {style : 'margin-top:1px;' },
		colspan : <%= colspan %>,
		items : [	
		<% if( HoValidator.isNotEmpty(title) ) { %>
			{ 
				xtype :'label_ux', 
...
<%
	if( buttons.length> 0 ) {
		...
		for( int i=0 ; i< buttons.length ; i++ ) {
			...
			out.print(  (i>0  ? "," :"" ) + " { xtype : 'button', border: 1, style: { borderColor: '#99bce8', borderStyle: 'solid'} ,  text : '"+ btnDisplayName + "'");
			if( !"random".startsWith(id)) { 
				out.print(", id : '"+ param.get("p_action_flag")  + "_" + id + "_btn_" + btnDisplayName +"'");
			}
			if( !"".equals(glyph)) {
